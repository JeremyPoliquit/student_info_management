...
router.post("/student/create", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: student_id, username: student_username },
      "your_secret_key",
      { expiresIn: "1h" }
    );
...
// student schedule
router.post("/student/schedule", async (req, res) => {
  const {
    student_number,
    course_code,
    sched_day,
  ...
  try {
    ...
    const insertRecordSQL = `
        INSERT INTO class_sched (course_code, sched_day, durationIn, durationOut, room, professor, student_id)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `;
    const recordValues = [
      course_code,
...
router.post("/record/create", async (req, res) => {
  const { student_number, subjects, task, dates, scores } = req.body;

  // Ensure all required fields are provided
  if (!student_number || !subjects || !task || !dates || !scores) {
    return res.status(400).json({ message: "All fields are required" });
  }
  ...
  try {
    ...
    // Insert into records table
    const insertRecordSQL = `
        INSERT INTO records (subjects, task, dates, scores, student_id)
        VALUES (?, ?, ?, ?, ?)
      `;
    const recordValues = [subjects, task, dates, scores, student_id];
